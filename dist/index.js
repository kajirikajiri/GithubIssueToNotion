...
const run = () => {
    try {
        ...
        const notionDatabaseId = core.getInput("notion-database-page-url").split('/')[4].split('?')[0];
        const issueTitle = core.getInput("issue-title");
        const issueNumber = core.getInput("issue-number");
        const issueState = core.getInput("issue-state");
        const issueLabels = JSON.parse(core.getInput("issue-labels"));
...
function getPropertiesFromIssue(issueTitle, issueNumber, issueState, numberOfComments, issueUrl, issueLabels) {
    return {
        ...
        Labels: {
            multi_select: issueLabels.map(l => { name: l.name; })
        }
    };
}
